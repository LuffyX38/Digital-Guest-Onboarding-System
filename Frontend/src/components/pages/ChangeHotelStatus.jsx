...
export default function ChangeHotelStatus() {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (!status) return console.log("nothing to change");
    // console.log(status);
    fetch(`http://localhost:3000/api/v1/hotel/config-hotel/${hotel_name}`, {
      method: "PATCH",
      credentials: "include",
      headers: {
            ...
            <div className="mt-4 col-start-1 row-start-3 self-center sm:mt-0 sm:col-start-2 sm:row-start-2 sm:row-span-2 lg:mt-6 lg:col-start-1 lg:row-start-3 lg:row-end-4">
              <div>
                <Menu as="div" className="relative inline-block text-left">
                  ...
                  <MenuItems
                    transition
                    className="absolute left-0 z-10 mt-2 w-56 origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in"
                  >
                    <div className="py-1">
                      <MenuItem>
                        <button
                          className="block w-full px-4 py-2 text-left text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:text-gray-900 data-[focus]:outline-none"
                          onClick={() => setStatus("Under maintainance")}
                        >
                          Under maintainance
                        </button>
                      </MenuItem>
                ...
                <button
                  type="submit"
                  className="px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mx-3"
                  onClick={handleSave}
                >
                  Save
                </button>
              </div>
              <small>{status ? `change to ${status}` : ""}</small>
              {/* <button
                type="button"
